...
Connects to a snapserver via python-snapcast (asyncio JSON-RPC), spawns a
local snapclient child process for audio playback, and publishes an
MPRIS2 interface on D-Bus via dbus-fast. Everything runs in a single
asyncio event loop — no threads, no GLib.
"""
...
SNAPSERVER_STREAMING_PORT = 1704
...
def discover_snapserver() -> tuple[str, int] | None:
    """Return ``(host, streaming_port)`` for a snapserver discovered via
    Zeroconf, or ``None`` if discovery fails."""
    zc = Zeroconf()
    try:
...
# --- Daemon ------------------------------------------------------------
async def run(host: str, control_port: int, streaming_port: int, bus_type: BusType) -> None:
    ...
    snapclient_path = shutil.which(SNAPCLIENT_BIN)
    if snapclient_path is None:
        logger.critical("%s not found in PATH (Debian: install the snapclient package)",
                        SNAPCLIENT_BIN)
        sys.exit(1)
    proc = await asyncio.create_subprocess_exec(
        snapclient_path, "-e", "-h", host, "-p", str(streaming_port),
        stdout=asyncio.subprocess.DEVNULL,
        stderr=asyncio.subprocess.DEVNULL,
    )
    logger.info("snapclient started from %s (pid=%d)", snapclient_path, proc.pid)

    # Match our MAC to find our snapclient in the snapserver's roster.
    # snapclient takes a moment after launch to register, so retry once.
    ...
    if client is None:
        ...
        client = identify_client(server, macs)
    if client is None:
        logger.warning("local MAC %s not in snapserver roster; controls "
                       "and metadata will be inert until the client registers", macs)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        description=("MPRIS2 D-Bus bridge for the local Snapcast client. "
                     "Spawns snapclient, talks to snapserver, exposes "
                     "PlaybackStatus and track metadata over D-Bus."),
    )
